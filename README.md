...
5. Go to File -> Export Per-waveform Data. Select "Matlab file" as Format.
  ...
  *. Choose All Channels, All Units, One file per channel.

  *. Check "Import unit designation", select a designation. Usually, there's only one choice.

  *. Removing check marks from PC1 and PC2 can save some disk space.

  *. Can write out to toPlexonOfflineSorter/sessionXX/subsessionZ/sessionXX.mat, which is the same directory as the input directory.

  *. The output files are "sessionXX_YYY.mat" where "_YYY" indicate channel numbers. Each of this file contains wvf0 (matrix).

  *. Sometimes, an error message shows up saying "the file is already opened by Microsoft Excel". In that case, make a new directory and write out there.
